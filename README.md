...
Character design supports GLB-inspired base model buttons, visible prefab export/import, outfit/accent/hair swatches, accessory toggles, and body-shape controls for height, shoulders, chest, arms, legs, hands, feet, head, and mass. The advanced Character Studio is an RPG-maker-style modeling workspace with draggable morph sliders, precise steppers, per-field and whole-model reset, undo, live color swatches, model measurements, front/profile/back views, and full-body/face framing. Mouse and controller can operate the complete workflow. Confirming stores an editable character blueprint; body proportions feed the visible character, collider size, movement tuning, stamina, armor capacity, and health.
