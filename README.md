...
- Starfall Forge ET3e foundation: the in-game level workspace provides controller-focused authoring controls, transactional transforms, stable world adapters, atomic project/recovery saves, draft publishing, per-record source files, cross-file hash recovery, and a scrollable content registry. Material records expose six typed shader families, presets, bounded parameters, live previews, publish validation, stable runtime catalogs, and persistent scene-object material bindings.
