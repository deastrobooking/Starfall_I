...
- Starfall Forge ET3c foundation: the in-game level workspace provides controller-focused authoring controls, transactional transforms, stable world adapters, atomic project/recovery saves, draft publishing, per-record source files, cross-file hash recovery, and a scrollable content registry with search, validation, dependency-safe rename/delete, duplication, canonical source moves, and seeded toon-material recipes.
- Rendering R1 foundation: branchless scene-lit toon bands, light-masked anime rims, configurable GPU grass wind/root gradients, procedural river/ocean water, and compiled Energy/Shield preview materials use Bevy 0.19 shader contracts and are exercised in the Forge preview gallery.
