...
- Humanoid motion has a new agent-facing roadmap in `docs/motion_mechanics_roadmap.md`; the first implementation slice expands procedural poses for fall, flight, and one-hand wall slides, and wall sliding now acts as a stamina-backed wall clasp with a tired-slide fallback.
