...
|---|---|---|
| Boot and world-load path | **Ported in this slice** | Save selection now runs once before the initial state transition and is cached for gameplay hydration; chapter travel displays an immediate loading card. Shared primitive caches retain the authored road/city/nature layout while cutting the fixed-seed startup from 108,755 to 9,257 queued meshes, and grass streaming is hard-capped at two fresh meshes / 24,000 candidate vertices per frame. Both fixed and legacy motor boot smokes complete without touching the rotating saves. |
...
| Independent Heavy vehicle simulation and online play | **Ported domain/runtime foundation / Separate secure networking program** | The exact two-kind/four-preset Heavy fleet has durable independent transforms, access leases, mount/eject, HP/destruction, swept movement/AABB contacts, turbo/orbital cruise, respawn, and complete Ghost Ride effects. A fixed-tick Bevy bridge now binds stable local operators and emits typed spawn/mount/motion/damage/detonation commands without double-driving native transforms or combat; an Avian/ECS presentation and shared-damage consumer still has to mount those records physically. Authoritative online play stays a separate security program rather than copying Heavy's trusted-client prototype. |
...
| Vehicle designer/factory | **Ported domain/runtime foundation** | Vehicle Forge now authors and publishes runtime-neutral Boat, Car, Truck, and Motorcycle recipes with a procedural preview and read-only Game catalog. Gameplay motor/physics adapters remain: published designs do not yet replace the existing robot-assembly, authored-boat, or Heavy Water vehicle runtimes. |
...
| Heavy feature panels and first-run guide | **Remaining** | The existing owner-aware Workshop has live Crafting, Jewels, Bio Garden, and Market Uplink tabs, including per-player cursors and canonical inventory/wallet transactions. All 11 sourced field-manual sections/41 rows and durable first-run completion now exist behind typed presentation messages. Build preview, full Bio Dex/creature care, companion roster/upgrades, live-map rendering, and the actual guide panel remain. |
...
| Exact Heavy primary balance and weapon-part upgrade table | **Ported domain/runtime foundation** | All eight source identities (six primaries, Tracking Missile, and Capture Net), level 1–5 stats, costs, global source rules, and direct Starfall rank 0–4 mapping are typed and tested. Atomic canonical purchase adapters preserve Starfall inventory/wallet authority; selecting which exact balance values replace native Starfall damage/fire behavior remains deliberate runtime integration. |
...
| Glaive, twin daggers, plasma axe, and chain whip | **Ported domain/runtime foundation** | All four exact catalogs, twelve unlock costs, combo patterns, timed hit descriptors, and specials are owner-keyed and atomically purchasable from canonical materials. They still need move-library/Weapon Forge execution and presentation rather than a parallel monolithic combat system. |
...
| Elemental active specials | **Ported domain/runtime foundation** | Lightning, Ice, Fire, Wind, Psychic, and Laser retain exact level 1–20 scaling, target/dome patterns, breakpoints, costs, and deterministic per-player session cooldowns. The runtime bridge ticks those cooldowns and atomically buys levels; casting descriptors still need routing through Starfall's shared damage/VFX pipeline and input/UI. |
...
| Heavy armor-capsule encounter placement | **Ported domain/runtime foundation** | The six exact capsule identities, tiers, costs, granted pieces/infusions, applied-ID persistence, and atomic purchase effects are ported. Physical Discoverable placement and deliberate mapping into native armor/mobility remain; load never replays one-shot grants or double-counts source-declared defense strings. |
...
| Independent vehicle collision, mounting, damage, and destruction | **Ported domain/runtime foundation** | The exact Heavy fleet owns independent transforms, 200 HP, destruction, party/owner access, one atomic operator lease, <5.5-unit nearest mount, exact eject/respawn, hitbox descriptors, and swept wall-contact motion. `HeavyVehiclePlugin` transactionally handles typed requests, validates deterministic terrain/wall probes, advances the fleet at fixed tick, and emits inert consumer commands. Mapping those commands onto Bevy/Avian entities, vehicle cameras, and shared combat remains. |
...
| Ghost Ride, ramming, and general mounted-weapon amplification | **Ported domain/runtime foundation** | Ghost Ride now preserves the exact ATV/fighter speed floors, locked heading, six-second fuse, swept ground/wall/target triggers, side/up eject, and typed radius-18/420-damage falloffs without ever damaging the player. The shared explosion/damage and ECS presentation consumers remain; ordinary mounted-weapon amplification is cataloged but not silently applied to Starfall weapons. |
...
| Recurring multi-site invasion scheduler and Heavy enemy fortresses | **Ported domain/runtime foundation** | Exact 900/1200-second cadence, 120-second warning, campaign floor/gates, all 11 zones (nine eligible), source threat/selection/resolution rules, Swarms General hook, and strategic rewards are save-ready. The Playing-gated bridge emits advisory typed encounter/checkpoint descriptors while leaving Starfall's campaign/RaidRegistry authoritative; physical fortress/drone waves remain authored consumers. |
...
| Friendly NPC dialogue | **Native/superseded + ported continuity content** | Starfall's discussion NPC/panel and radio/subtitle systems remain authoritative. Heavy's exact six named NPCs, six palettes, 24 lines, and 5.5/9-unit acquire/release hysteresis are now typed behind stable-player request/presentation messages; physical NPC placement feeds the native panel later. |
...
| New feature persistence | **Ported in this slice** | Schema v5 persists the timed crafting queue, economy owners/vendors/builds/mining/chests/jewel sockets, Bio Dex/gardens/companions/rescues plus deterministic clock, combat unlocks, the independent vehicle fleet, legacy region/prop records, and campaign/player-scoped world events/manual progress. Legacy saves default safely; no ECS `Entity` IDs are serialized. |
