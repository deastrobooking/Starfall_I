...
| Independent Heavy vehicle simulation, legacy regions, and online play | **Remaining** | The new vehicle bodies follow the authoritative player/mode and are not independently colliding, damageable, destructible, or world-mountable vehicles. Ghost Ride/ram-detonation, Heavy's named regional lifecycle, and authoritative online play are still absent. Detailed boundaries remain below. |
...
| Heavy feature panels and first-run guide | **Remaining** | Add owner-aware build, bio garden/Dex, companion roster/upgrades, jewel socket, vendor, and live-map panels inside existing Bevy UI. A first-run tutorial/guide still needs a durable completion flag. |
...
| Heavy legacy regions and side-zone lifecycle | **Remaining** | Preserve Starfall's 14-chapter campaign. Add a separate stable region/session layer for Detroit, Ashur Sanctuary, Orbital Front, Pontiac Lab, Swarm's Lair, Saginaw Lab, Zug Island, Ann Arbor, and Michigan Wilds, with explicit mount/despawn ownership, return anchors, atmosphere, and save keys. Do not reinterpret Heavy's numeric levels as Starfall chapters. |
...
| Weather, regional time/tint, and space presentation | **Remaining** | Add a `WorldAtmosphere` resource consumed by the existing day/night system, with time, cycle rate, sky tint, weather, and space-mode fields selected by active region. |
...
| Damageable environment prop clusters | **Remaining** | Add typed crate/barrel/canister/container/hologram props using `Health`, `Damageable`, seeded placement, staged presentation, canonical loot, and `SpatialLod`. |
...
| Ashur farming and animal clinic | **Remaining** | Passive settlement Farm output is not equivalent to planting, growth, harvesting, seed returns, pet care, and clinic rehabilitation. Add a separate save-backed garden/farming loop. |
...
| Freeform block building and mining placed blocks | **Remaining** | Heavy has 19 costed block types, 2 m snap, preview, rotation, overlap validation, damage/mining/refunds, and export/import. Add a game-side player-build registry, owner-aware placement intents, physical validation, resource transactions, health, and save records. Forge/editor scene drafts are an authoring substrate, not player builds. |
| Runtime prefab placement | **Remaining** | Heavy has 15 fixed prefab recipes across Defense, Housing, Industry, and Decoration. Compile equivalent data through Starfall's procedural building/catalog path and persist stable prefab ID/version plus transform and state. |
...
| Heavy materials and capacity policy | **Remaining** | Add stable definitions and real uses for gear, bio essence, weapon-specific parts, seeds/crops/feed, and jewels. Decide whether 24 or 100 slots fits Starfall; never silently lose rare or quest drops. |
...
| Complete crafting catalog and outcomes | **Remaining** | Replace placeholder recipes/results with data-driven Heavy-intent recipes. Base, robot, drone, city, ship, and vehicle outputs must invoke their owning subsystem rather than become inert inventory tokens. Resolve result stack limits from item definitions and define queue save behavior. |
...
| Heavy drop tables, jewels, and companion auto-loot | **Remaining** | Extend canonical loot tables, add rare-drop presentation, weapon parts, Power Jewels/Star Cores, and owner-attributed companion collection. |
| Destructible mining nodes | **Remaining** | Add scrap piles, crystals, bio pods, and gear caches as typed nodes using the shared damage pipeline, `GameRng`, authored spawn data, loot events, and respawn state. |
| Weapon jewel sockets | **Remaining** | Add a per-player stable weapon-socket ledger; rough/cut/flawless provide the intended damage tier only once in the canonical damage calculation. Mount/swap/unmount must be atomic and protect refunds when inventory is full. |
...
| Chest completion and presentation | **Remaining** | Ammo and weapon-upgrade reward branches are currently no-ops. Finish their semantics and add open animation/VFX/SFX, terrain-aware placement, and stable one-shot IDs where chapter persistence requires them. |
...
| Material vendor, selling, stock, and restock | **Remaining** | Add a separate owner-scoped atomic material/vendor layer without weakening durable ownership or duplicate protection. Decide whether finite stock is personal or campaign shared. |
...
| Personal companion roster, cap, upgrades, death/revive, and save hydration | **Remaining** | Persist a per-player roster and active selection, spawn that roster instead of unconditional defaults, implement level/weapon-tier transactions, damage/death/revive, and use assembled creature/robot visuals. Keep campaign-shared robot pets distinct unless ownership is deliberately redesigned. |
| Bio species catalog | **Remaining** | Reuse versioned CreatureSpec visuals, but add stable gameplay species definitions for Heavy's 133 entries, 24 archetypes, 11 elemental types, rarity, stats, and capture odds. |
| Wild spawning, capture, Dex, care, bond, and passive bonuses | **Remaining** | Add owner-neutral wild entities and per-player capture intent/result, permanent Dex, capped roster, feeding/care/level/bond, active pet selection, and capped derived bonuses. Either implement weakening as a real rule or remove that tutorial claim. |
| Bio garden and farming | **Remaining** | Add save-backed garden tier/cap/bonus, twelve plot states, planting, growth, harvest, seed returns, crop/feed inventory integration, and owner-aware UI. |
...
| Synthetic and animal rescues | **Remaining** | Add stable caged-rescue definitions, interaction/story/shatter presentation, persistent completion IDs, four lab-animal milestones, and an idempotent rescue-all/general-defeat reward. Reuse Discoverable and Discussion infrastructure. |
...
| Live minimap/full gameplay map | **Remaining** | Chapter select and fast travel exist, but Heavy's live player/enemy/shop/garden/chest/base/boss marker view and off-screen arrows do not. Add a typed map-marker registry and make the split-screen ownership policy explicit. |
| Local saves, autosave, recovery, and migration | **Native/superseded** | Starfall's versioned v4 schema, atomic rotating generations, corrupt-save fallback, legacy defaults, 30-second autosave, per-player ordering, and shared registries are substantially safer than Heavy's optional-field JSON snapshot. |
| New feature persistence | **Remaining** | Bump/migrate the schema for stable player-build records, personal companion rosters, weapon sockets, captured Dex/garden, rescue IDs, legacy regions, and any physical vehicle state worth retaining. Never serialize ECS `Entity` IDs. |
...
| Local versus arena and match rules | **Remaining** | An offline PvP test arena can be built from factions, damage rules, spawn points, per-player HUD, score, timer, and win state without networking. Heavy's current arena lacks a complete match loop. |
...
- `PrefabSystem.ts` is a fixed registry of 15 recipes. `BaseSystem` persists
