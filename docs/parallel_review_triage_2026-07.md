...
| Saves overwrite all players with P1 or permit NaN armor | **Mostly stale.** `players[]` is keyed by `player_index`, values are bounded when applied, and legacy flat fields are compatibility fallback only when `players[]` is empty. Inventory stacks and active loadout choices now round-trip per player. | Superseded after this snapshot: schema v4 recovery and per-player `PlayerProgression` are delivered; top-level perk/upgrade/rank fields are compatibility mirrors. |
